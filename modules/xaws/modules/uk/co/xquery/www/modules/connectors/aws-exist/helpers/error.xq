...
(:~
 ...
 :)
module namespace error = 'http://www.xquery.co.uk/modules/connectors/aws/helpers/error';
        ...
        then $http_response[1] 
            ...
            typeswitch ($http_response[2])
                ...
                case $resp as xs:base64Binary 
                    return 
                        util:base64-decode($http_response[2])
...
declare function error:to-string($code,$message,$obj) as xs:string {
    let $msgs := 
        ( 
            ...
            if($obj)
            then concat( "Errorobject: ", serialize($obj))
            else "Errorobject: ()"
        )
